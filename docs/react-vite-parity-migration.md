...
과거 HTML snapshot/reference 라인은 더 이상 기본 개발, 검증, APK 빌드에 사용하지 않는다. 관련 실행 파일과 추출 도구는 저장소 혼동을 줄이기 위해 정리했다.
...
과거 도구 파일은 저장소에서 제거했다. 신규 기능 작업과 APK 검증은 React/Vite 단일 라인만 기준으로 사용한다.
