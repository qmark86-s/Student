...
- `src/react/game/expedition.js`
  - 직업 동료 기반 원정대, 5인 파티, 실시간 역할 전투, 자동 정산, 즉시/pending 보상, stage 진행, 성장 투자, 연구, 승급 합성, 파견/의뢰를 담당한다.
...
- 원정대: 챕터 10개, stage 100개, 보스 100개, 직업 역할 전투 스탯 62개, 일반/보스 전투 enemy 설정 각 100개, 유닛 레벨 600개, 연구 노드 18개, 파견 의뢰 12개
...
- 보스 첫 클리어는 다이아/EXP/연구 재화와 `claimedBossStages`를 기록하며, 이미 claim된 보스는 다이아나 연구 재화를 중복 지급하거나 pending에 누적하지 않는다.
- 보스 패배는 해당 구간 첫 stage로 회귀하고, 일반 stage 패배는 현재 stage를 유지한다.
- 오프라인 pending 보상의 EXP/부동산 자금/다이아/연구 재화는 보상 팝업의 `받기` 수령 시 실제 재화로 지급된다.
- 파티/관리 목록에는 역할 필터가 있고, `추천 편성`은 전투력순 상위 5명을 편성한다.
- 원정대 관리 탭은 `성장 / 연구 / 파티 / 의뢰 / 대원 관리 / 기록` 순서다.
- 연구 데이터는 `data/expedition_research.json` 기준이며 정적 lane/노드만 사용한다. 런타임에서 연구 노드나 효과를 생성하지 않는다.
- 연구 트리는 5개 lane의 세로 룬 트리이며, 보급/지휘/작전/공격/의무·보상 갈래가 교차 선행 조건으로 합류한다. UI는 원형 노드와 SVG 연결선을 중심으로 표시하고, 선택한 연구 1개만 compact 상세 패널에서 확인한다.
- 연구 탭은 상단 원정대 전투 씬을 렌더하지 않아 룬 트리가 첫 화면의 주 콘텐츠가 된다. 파티/의뢰/대원 관리/기록 등 다른 원정대 탭은 동일한 전투 씬 크기를 유지한다.
- 연구 재화는 원정대 중간보스 첫 클리어 1점, 챕터보스 첫 클리어 3점을 기본 지급한다. 이벤트/상점 지급은 같은 `save.expedition.research.points` 필드를 쓰는 후속 확장 대상이다.
- 연구 효과는 모든 원정대원에게 적용되며 역할 전용 효과는 해당 역할의 실제 전투 스탯에만 적용한다. `partyPowerPercent` 합계는 v1 기준 `rules.maxTotalCombatBonus=0.2`를 넘지 않는다.
- 연구 초기화는 무료이며 완료 노드를 비우고 투자한 연구 재화를 전액 반환한다.
...
- 의뢰 v1 실제 지급 보상은 원정대 EXP, 다이아, 부동산 자금만 처리한다. 의뢰 카드의 테크트리 포인트, 유물, 원정대 뽑기권은 UI에 `준비중`으로 표시하되 저장/지급하지 않는다.
...
- `implementations/expedition-research-tech-tree/implementation.md`
- `implementations/project-commercial-quality-audit/implementation.md`
