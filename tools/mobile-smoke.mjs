...
const outputDir = resolve("artifacts/mobile-smoke");
const preferredPort = Number(process.env.MOBILE_SMOKE_PORT || 5320);
const viewports = [
...
try {
  for (const viewport of viewports) {
    ...
    if (metrics.textLength < 100) failures.push("Rendered text is unexpectedly short");
    if (metrics.buttonCount < 5) failures.push("Interactive controls are unexpectedly sparse");
...
if (failed.length > 0) {
  console.error(`MOBILE_SMOKE_FAILED ${failed.length}/${results.length}`);
  process.exit(1);
}

console.log(`MOBILE_SMOKE_OK screenshots=${outputDir}`);
