...
// 배틀로드/교과 VFX config는 더 이상 번들에 baked되지 않는다.
// 패치는 토큰(__BATTLE_ROAD_CONFIG_JSON__ / __CURRICULUM_ATTACK_VFX_JSON__)만 심고,
// 실제 데이터는 빌드 시점(tools/snapshot-build.mjs)에 data/*.json에서 주입한다.
// 덕분에 data/ 파라미터를 편집해도 추적 대상 app.bundle.js가 더럽혀지지 않는다.
...
replaceAnyExact(
  ...
  "y={\"--student-scale\":_.toFixed(3)},be=battleRoadVisualPhase(c),x=l?.kind===`boss`||l?.kind===`suneung`,S=c.kind===`suneung`?`suneung`:`term`,",
