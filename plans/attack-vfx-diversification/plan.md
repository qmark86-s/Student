...
- 학생 캐릭터 `::after` 베기 VFX에서 노랑/빨강 계열을 제거하고, 흰색/청록 계열의 짧은 학습 타격선으로 낮춘다.
- 학생 `::before` 먼지는 공격 투사체처럼 보이지 않도록 크기와 opacity를 줄인다.
- `battleDustBurst`, `battleImpactFlash`의 노랑 계열을 제거해 불꽃/폭발처럼 보이지 않게 한다.
- 교과 토큰 시작 Y offset은 학생 상체/손에서 출발하도록 0 이하로 유지한다.
- `validate-battle-road-config`, `verify-visual-assets`, `visual-asset-smoke`에 불꽃색/바닥 출발 회귀 검증을 추가한다.
- 검증은 `curriculum-vfx:verify`, `battle-road:verify`, `visual:verify`, `build:web`, `visual:smoke`, 가능하면 `verify:mobile`까지 수행한다.
