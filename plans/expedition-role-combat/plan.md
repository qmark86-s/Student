...
- 공속은 정책값으로 고정한다. 탱커 3.0, 딜러 1.5, 힐러 1.0, 몬스터 2.0이며, 공속 성장값은 0으로 둔다.
- 공속 1.0은 초당 1회, 공속 1.5는 초당 1.5회, 공속 3.0은 초당 3회 행동한다.
...
- 마지막 전투 리포트의 `enemyHp`, `partyHp`, `enemyDefeatOrder`, 이벤트별 HP before/after를 승패와 무관하게 화면에 반영해 부분 처치 실패를 표현한다.
- 원정대 몬스터 리소스는 모든 Stage/보스의 asset id가 실제 `src/snapshot/assets/individual/expedition-enemies/<asset>/move_0..3.png`를 갖는지 검증한다.
- Stage 이동은 모든 몬스터 처치/디스폰 연출이 끝난 뒤에만 시작하고, 이동 중에는 이전 Stage 몬스터를 렌더하지 않는다. 이동 시간은 4초로 둔다.
- 다음 몬스터 접근은 이동 완료 후 2.95초 조우 연출로 분리하며, 조우가 시작되면 파티 이동 모션은 멈춘다.
- 실행 중 자동 전투는 전투 정리, Stage 이동, 몬스터 조우 접근이 끝나 실제 원정대 화면의 `combatReady`가 true가 된 뒤에만 다음 전투를 시작하며, 한 번에 1전투만 처리한다.
- 원정대원 표시 모션은 `combat`, `running`, `standing`으로 분리한다. 전투/정리/일반 대기 중에는 기존 이동 스프라이트를 제자리 걷기처럼 쓰는 `combat`, Stage 이동 중에는 `running`, 조우 진입이나 미편성 상태에는 `standing`을 사용한다.
- 별도 idle 스프라이트는 현재 확인되지 않았으므로, 전투 중 정지 대신 기존 걷기 프레임을 제자리 루프로 재생한다.
- 밸런스 검증은 역할별 공속 정책과 몬스터 공속 2.0, 공속 성장 0을 함께 확인한다.
