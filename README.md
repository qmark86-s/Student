...
2026-06-25 현재 `npm run react:verify`, `npm run verify:mobile`, `rg -n '\?\?|fallback|Fallback|unknown' src/react -S`, `git diff --check`, `mcp__UmgMcp.project_policy_gate` strict는 통과 상태다. `react:verify`는 부동산 도시 맵/지역별 리소스 데이터 검증, 부동산 smoke, 부동산 10개 지역 전수 시각 감사를 포함하며, `react:responsive-audit`는 320x568부터 1280x720까지 8개 viewport 실패 0건이다. `src/react` 런타임 소스는 금지 대체 토큰 검색 0건이다.
