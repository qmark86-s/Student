...
- 원정대 배경 PNG의 좌우 루프 seam을 블렌딩해 `repeat-x` 사용 시 경계가 튀는 문제를 줄였다.
...
- `src/react/App.jsx`
  ...
  - Stage 배경 offset은 `Stage 88px + 챕터 640px` 누적값으로 계산한다.
  ...
  - `combatReady` 상태를 계산해 `전투 정리중`, `이동중`, `조우중`에는 `data-combat-ready="false"`를 노출한다.
  - App의 온라인 자동 원정대 정산 interval은 화면의 `.expedition-scene[data-combat-ready]`를 기준으로 `전투 정리중`, `이동중`, `조우중`에는 실행하지 않는다. React ref만 기준으로 삼으면 디버그 대원 추가처럼 원정대 화면이 뒤늦게 열리는 경로에서 ref와 실제 DOM 상태가 어긋나 자동 전투가 영구 대기할 수 있어, DOM의 표시 상태를 최종 게이트로 사용한다.
...
- `src/react/styles.css`
  ...
  - 다음 Stage 몬스터 접근은 이동 완료 후 `.expedition-enemy-group.approaching`으로 2.95초 동안 재생한다.
  - 전투 정리중의 처치 몬스터는 `expeditionEnemyDeath`로 완전히 디스폰된다. Stage 이동 중에는 이전 몬스터 그룹을 숨기는 안전 CSS도 둔다.

## 배경 루프
- `tools/build-visual-assets.mjs`
  - `blendHorizontalLoopSeam(target, width)`를 추가했다.
  - `buildExpeditionBackdropAtlas()`가 내부 seam 블렌딩 후 좌우 루프 seam을 96px 폭으로 블렌딩한다.
- `src/snapshot/assets/visual-expedition-backdrops.png`
  - 현재 원정대 배경 PNG도 같은 seam 처리 결과로 갱신했다.
  - 좌우 edge diff는 0으로 확인했다.
...
- `tools/react-vite-expedition-smoke.mjs`
  ...
  - Stage 1 -> 2 배경 이동 offset이 `0 -> -88`인지 검사한다.
  - 현재 Stage 적의 개별 처치 order/delay를 검사한다.
  - 이동 중에는 조우 접근이 없고, 이동 완료 후 2.95초 접근 상태가 생기는지 검사한다.
  - `data-combat-ready`가 전투 정리중, 이동중, 조우중에는 false이고 조우 완료 뒤 true로 돌아오는지 검사한다.
...
## 남은 작업
- 실제 기기에서 4초 Stage 이동 시간, 2.95초 조우 접근, 플로트 체류 시간이 답답하거나 빠르게 느껴지는지 플레이 감각 조정이 필요하다.
