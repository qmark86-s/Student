...
## 개요
- Stage 전환 시간은 4초로 늘리고, Stage당 배경 이동 거리는 88px로 유지해 이동 체감을 낮췄다.
...
- 다음 Stage 몬스터를 전환 종료 시점에 갑자기 교체하지 않고, 이동 완료 후 오른쪽에서 접근하는 조우 연출로 보여준다. 조우가 시작되면 파티 이동 모션은 잠깐 `standing`으로 멈추고, 이후 전투/대기 `combat` 제자리 걷기로 돌아간다.
...
- `src/react/App.jsx`
  - `EXPEDITION_STAGE_BACKDROP_STEP_PX = 88`로 조정했다.
  ...
  - Stage 이동 중에는 이전 Stage 적 그룹을 렌더하지 않고, 이동 완료 후 새 Stage 적 그룹만 `.expedition-enemy-group.approaching`으로 표시한다.
  ...
  - 파티 visual은 Stage 이동 중에만 `running`, 전투 정리/대기 중에는 `combat`, 조우 진입 중에는 `standing`이 된다.
  - 이동 완료 후 현재 Stage 적 그룹은 `.expedition-enemy-group.approaching`으로 오른쪽에서 접근한다.
- `src/react/styles.css`
  ...
  - `expeditionEnemyEncounterApproach`를 추가해 새 몬스터가 이동 완료 후 오른쪽에서 접근한다.
  - 접근 animation duration은 2.95초로 둬 기존 접근 속도의 2/5가 되게 했다.
...
- `tools/react-vite-expedition-smoke.mjs`
  - Stage 1 -> 2 배경 offset이 `0 -> -88`인지 검사한다.
  - 전환 후 Stage 2 offset이 `-88`에 유지되는지 검사한다.
  ...
  - 전환 중에는 접근 그룹이 없고, 이동 완료 후 현재 몬스터 그룹이 2.95초 접근 animation을 갖는지 검사한다.
  - 파티가 전투 정리/대기 중 combat, 이동 중 running, 조우 중 standing인지 검사한다.
...
## 남은 작업
- 실제 기기에서 4초 이동, 88px 이동 거리, 2.95초 몬스터 접근이 충분히 자연스러운지 감각 튜닝이 필요하다.
